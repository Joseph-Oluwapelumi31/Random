  ...
  return (
    <motion.div ref={ref} initial={{ opacity: 0, y: 28 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: index * 0.09 }} data-testid={`nih-project-${p.id}`} className="glass-card rounded-2xl border overflow-hidden hover:border-opacity-40 transition-all duration-300" style={{ borderColor: `${p.color}20` }}>
      ...
      <div className="p-5">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="font-heading font-bold text-foreground text-base">{p.name}</span>
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ color: p.color, background: `${p.color}15` }}>{p.agency}</span>
              </div>
              <p className="text-muted-foreground text-xs mt-0.5 max-w-xs truncate">{p.full}</p>
            </div>
          </div>
          <div className="text-right flex-shrink-0">
            <p className="font-heading font-bold text-lg" style={{ color: p.color }}>{fmt(p.total)}</p>
            <p className="text-muted-foreground text-xs">{p.years}</p>
          </div>
        </div>
        <div className="flex items-center gap-2 mb-3 flex-wrap">
          <span className="text-xs px-2.5 py-1 rounded-full glass-card border border-foreground/8 text-muted-foreground font-medium">{p.role}</span>
        </div>
        <div className="mb-3">
          <div className="flex justify-between text-xs text-muted-foreground mb-1.5"><span>Portfolio Share</span><span>{pct}%</span></div>
        ...
        {open && (
          <div className="mt-3 rounded-xl overflow-hidden border border-foreground/6">
            <table className="w-full text-xs">
              <thead><tr className="border-b border-foreground/6 bg-foreground/2"><th className="text-left px-3 py-2 text-muted-foreground font-semibold">Type</th><th className="text-left px-3 py-2 text-muted-foreground font-semibold">Year</th><th className="text-right px-3 py-2 text-muted-foreground font-semibold">Amount</th><th className="text-left px-3 py-2 text-muted-foreground font-semibold hidden sm:table-cell">NIH ID</th></tr></thead>
              <tbody>
                {(p.awards || []).map((a, i) => {
                  const s = PHASE_STYLE[a.type] || { color: '#94a3b8', bg: 'rgba(148,163,184,0.08)' };
                  return (
                    <tr key={i} className="border-b border-foreground/4 last:border-0 hover:bg-foreground/2">
                      ...
                      <td className="px-3 py-2.5 text-right font-semibold text-foreground">{fmt(a.amount)}</td>
                      <td className="px-3 py-2.5 text-muted-foreground font-mono hidden sm:table-cell">{a.id || '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
...
export default function NIHFunding({
  ...
  return (
    <section id="nih-portfolio" data-testid="nih-funding-section" className="py-20 px-6 section-divider">
      <div className="max-w-7xl mx-auto">
        <div ref={ref} className="mb-10">
          <motion.p initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} className="text-primary text-xs font-semibold uppercase tracking-widest mb-3">{eb}</motion.p>
          <motion.h2 initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.08 }} className="font-heading text-4xl sm:text-5xl font-bold text-foreground leading-none mb-8">
            {tm}{' '}
            <span className="text-gradient-cyan">{ta}</span>
          </motion.h2>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-2">
            {summaryTiles.map((s) => (
              <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.15 }} className="glass-card rounded-2xl border border-foreground/6 p-5 text-center hover:-translate-y-0.5 transition-transform duration-200">
        ...
        <p className="text-center text-muted-foreground text-xs mt-8">{foot}</p>
      </div>
    </section>
  );
}
